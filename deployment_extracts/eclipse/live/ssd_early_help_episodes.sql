...
-- =============================================================================
-- DROP TABLE IF EXISTS ssd_early_help_episodes;

-- META-ELEMENT: {"type": "create_table"}
CREATE TABLE IF NOT EXISTS ssd_early_help_episodes (
    earl_episode_id           VARCHAR(48) PRIMARY KEY,   -- metadata={"item_ref":"EARL001A"}
    earl_person_id            VARCHAR(48),               -- metadata={"item_ref":"EARL002A"}
    earl_episode_start_date   TIMESTAMP,                 -- metadata={"item_ref":"EARL003A"}
    earl_episode_end_date     TIMESTAMP,                 -- metadata={"item_ref":"EARL004A"}
    earl_episode_reason       TEXT,                      -- metadata={"item_ref":"EARL005A"}
    earl_episode_end_reason   TEXT,                      -- metadata={"item_ref":"EARL006A"}
    earl_episode_organisation TEXT,                      -- metadata={"item_ref":"EARL007A"}
    earl_episode_worker_id    VARCHAR(100)               -- metadata={"item_ref":"EARL008A", "item_status": "A", "info":"Consider for removal"}
);
        ...
        FROM (
           ...
           FROM (
               SELECT 
                   ...
                   EH.ENDREASON,
                   CASE 
                       WHEN EH.STARTDATE >= LAG(EH.STARTDATE) OVER (
                                PARTITION BY EH.PERSONID 
                                ORDER BY EH.STARTDATE, EH.ENDDATE NULLS LAST
                            )
                        AND EH.STARTDATE <= COALESCE(
                                LAG(EH.ENDDATE) OVER (
                                    PARTITION BY EH.PERSONID 
                                    ORDER BY EH.STARTDATE, EH.ENDDATE NULLS LAST
                                ),
...
FROM EH_EPISODE
LEFT JOIN CLOSURE 
       ON CLOSURE.PERSONID = EH_EPISODE.PERSONID
      AND (CLOSURE.COMPLETED_DATE = EH_EPISODE.EH_CLOSE_DATE 
           OR CLOSURE.CLOSURE_DATE = EH_EPISODE.EH_CLOSE_DATE) 
LEFT JOIN LATERAL (
           SELECT
               *
           FROM REFERRAL
...
LEFT JOIN WORKER 
       ON WORKER.PERSONID = EH_EPISODE.PERSONID
...
LEFT JOIN TEAM 
       ON TEAM.PERSONID = EH_EPISODE.PERSONID 
