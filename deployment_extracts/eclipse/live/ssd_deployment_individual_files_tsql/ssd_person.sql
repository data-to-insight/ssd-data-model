...
SELECT DISTINCT
    CONVERT(NVARCHAR(48), P.CAREFIRSTID)  AS pers_legacy_id,
    CONVERT(NVARCHAR(48), P.PERSONID)     AS pers_person_id,
    CONVERT(NVARCHAR(13), U.UPN)          AS pers_upn,
    CONVERT(NVARCHAR(100), P.FORENAME)    AS pers_forename,
    CONVERT(NVARCHAR(255), P.SURNAME)     AS pers_surname,
    CASE
    ...
    CONVERT(NVARCHAR(48), P.NHSNUMBER)    AS pers_single_unique_id,
    COALESCE(
        U.UPN,
        UU.UN_UPN,
        CASE
    ...
    NULL                                  AS pers_send_flag,
    CASE
        WHEN P.DUEDATE >= SYSDATETIME()
            THEN P.DUEDATE
    END                                   AS pers_expected_dob,
    P.DIEDDATE                            AS pers_death_date,
    CASE
        WHEN M.PERSONID IS NOT NULL THEN 'Y'
        ELSE 'N'
    END                                   AS pers_is_mother,
    CONVERT(NVARCHAR(48), P.COUNTRYOFBIRTHCODE) AS pers_nationality
