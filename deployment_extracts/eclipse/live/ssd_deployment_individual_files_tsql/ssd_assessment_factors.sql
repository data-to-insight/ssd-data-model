...
IF OBJECT_ID('tempdb..#ssd_assessment_factors', 'U') IS NOT NULL DROP TABLE #ssd_assessment_factors;
...
;WITH NEF AS (
    SELECT DISTINCT
        ...
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_alcoholChild')                    THEN FAPV.ANSWERVALUE END) AS ALCOHOL_CHILD,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_alcoholOtherPersonHousehold')     THEN FAPV.ANSWERVALUE END) AS ALCOHOL_OTHER_PERSON_HOUSEHOLD,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_alcoholParent')                   THEN FAPV.ANSWERVALUE END) AS ALCOHOL_PARENT,

        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_domesticChild')                   THEN FAPV.ANSWERVALUE END) AS DOMESTIC_CHILD,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_domesticOtherPersonHousehold')    THEN FAPV.ANSWERVALUE END) AS DOMESTIC_OTHER_PERSON_HOUSEHOLD,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_domesticParent')                  THEN FAPV.ANSWERVALUE END) AS DOMESTIC_PARENT,

        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_drugChild')                       THEN FAPV.ANSWERVALUE END) AS DRUG_CHILD,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_drugOtherPersonHousehold')        THEN FAPV.ANSWERVALUE END) AS DRUG_OTHER_PERSON_HOUSEHOLD,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_drugParent')                      THEN FAPV.ANSWERVALUE END) AS DRUG_PARENT,

        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_learnDisChild')                   THEN FAPV.ANSWERVALUE END) AS LEARN_DIS_CHILD,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_learnDisOtherPersonHousehold')    THEN FAPV.ANSWERVALUE END) AS LEARN_DIS_OTHER_PERSON_HOUSEHOLD,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_learnDisParent')                  THEN FAPV.ANSWERVALUE END) AS LEARN_DIS_PARENT,

        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_mentalHealthChild')               THEN FAPV.ANSWERVALUE END) AS MENTAL_HEALTH_CHILD,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_mentalHealthOtherPerson')         THEN FAPV.ANSWERVALUE END) AS MENTAL_HEALTH_OTHER_PERSON,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_mentalHealthParent')              THEN FAPV.ANSWERVALUE END) AS MENTAL_HEALTH_PARENT,

        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_physDisChild')                    THEN FAPV.ANSWERVALUE END) AS PHYS_DIS_CHILD,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_physDisOtherPersonHousehold')     THEN FAPV.ANSWERVALUE END) AS PHYS_DIS_OTHER_PERSON_HOUSEHOLD,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_physDisParent')                   THEN FAPV.ANSWERVALUE END) AS PHYS_DIS_PARENT,

        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_childSpecificFactor')             THEN FAPV.ANSWERVALUE END)      AS CHILD_SPECIFIC_FACTOR,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_childSpecificFactor')             THEN FAPV.SHORTANSWERVALUE END) AS CHILD_SPECIFIC_FACTOR_SHORT,

        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_allegedPerpetratorOfSexualAbuse') THEN FAPV.SHORTANSWERVALUE END) AS ALLEGED_PERP_SEXUAL_ABUSE,
        MAX(CASE WHEN FAPV.CONTROLNAME IN ('CINCensus_allegedPerpetratorOfPhysicalAbuse') THEN FAPV.SHORTANSWERVALUE END) AS ALLEGED_PERP_PHYS_ABUSE
    ...
    CONVERT(NVARCHAR(MAX),
        '[' + ISNULL(STUFF((
            ...
            FROM (VALUES
                ...
                (CASE WHEN NEF.CHILD_SPECIFIC_FACTOR_SHORT LIKE '%PFOther%' AND NEF.CHILD_SPECIFIC_FACTOR LIKE '%Privately fostered - Other%' THEN '8F' END),
                ...
                (CASE WHEN NEF.CHILD_SPECIFIC_FACTOR_SHORT LIKE '%PhysAbuse%' AND NEF.ALLEGED_PERP_PHYS_ABUSE LIKE '%18B%' THEN '18B' END),
                (CASE WHEN NEF.CHILD_SPECIFIC_FACTOR_SHORT LIKE '%PhysAbuse%' AND NEF.ALLEGED_PERP_PHYS_ABUSE LIKE '%18C%' THEN '18C' END),
                (CASE WHEN NEF.CHILD_SPECIFIC_FACTOR_SHORT LIKE '%SexAbuse%'  AND NEF.ALLEGED_PERP_SEXUAL_ABUSE LIKE '%19B%' THEN '19B' END),
                (CASE WHEN NEF.CHILD_SPECIFIC_FACTOR_SHORT LIKE '%SexAbuse%'  AND NEF.ALLEGED_PERP_SEXUAL_ABUSE LIKE '%19C%' THEN '19C' END),
                (CASE WHEN NEF.CHILD_SPECIFIC_FACTOR_SHORT LIKE '%Other%' AND NEF.CHILD_SPECIFIC_FACTOR NOT LIKE '%Privately fostered - Other%' THEN '20' END),
                (CASE WHEN
                    NEF.ALCOHOL_CHILD IS NULL
