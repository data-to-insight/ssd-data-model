...
LEFT JOIN [eclipseDelta].[dbo].[CLAPERIODOFCAREVIEW] CLA
    ON CONVERT(NVARCHAR(48), CLA.PERSONID) = CONVERT(NVARCHAR(48), CE.PERSONID)
   AND CONVERT(NVARCHAR(48), CLA.PERIODOFCAREID) = CONVERT(NVARCHAR(48), CE.PERIODOFCAREID)
OUTER APPLY (
    SELECT TOP (1) R.ASSESSMENTID
    FROM REFERRAL R
    WHERE R.PERSONID = CONVERT(NVARCHAR(48), CE.PERSONID)
...
OUTER APPLY (
    ...
    FROM IRO_MEETING I
    WHERE I.PERSONID = CONVERT(NVARCHAR(48), CE.PERSONID)
