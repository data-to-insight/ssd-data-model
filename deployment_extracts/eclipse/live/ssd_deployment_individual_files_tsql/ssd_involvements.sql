...
CIN_EPISODE_ORDERED AS (
    SELECT
        ...
        CASE
            WHEN b.startdate >=
                 LAG(b.startdate) OVER (
                     PARTITION BY b.personid
                     ORDER BY b.startdate,
                              CASE WHEN b.enddate IS NULL THEN 1 ELSE 0 END,
                              b.enddate
                 )
             AND b.startdate <= DATEADD(
                     DAY, 1,
                     ISNULL(
                         LAG(b.enddate) OVER (
                             PARTITION BY b.personid
...
CIN_EPISODE_GROUPED AS (
    SELECT
        o.*,
        SUM(o.next_start_flag) OVER (
            PARTITION BY o.personid
            ORDER BY o.startdate, o.episodeid
...
CIN_EPISODE AS (
    SELECT
        ...
        CASE
            WHEN MAX(CASE WHEN enddate IS NULL THEN 1 ELSE 0 END) = 1
                THEN NULL
            ELSE MAX(enddate)
...
LEFT JOIN CIN_EPISODE CE
       ON CE.cine_person_id = CONVERT(NVARCHAR(48), PPR.PERSONID)
      AND CAST(PPR.STARTDATE AS DATE) >= CE.cine_referral_date
      AND CAST(PPR.STARTDATE AS DATE) <
            ISNULL(CE.cine_close_date, CAST(GETDATE() AS DATE))
LEFT JOIN TEAM T
       ON T.personid = CONVERT(NVARCHAR(48), PPR.PERSONID)
      AND ISNULL(CAST(PPR.CLOSEDATE AS DATE), CAST(GETDATE() AS DATE))
            >= T.team_start_date
      AND CAST(PPR.STARTDATE AS DATE)
