...
IF OBJECT_ID('[SSD].[ssd_cla_previous_permanence]', 'U') IS NOT NULL
BEGIN
    IF EXISTS (
        SELECT 1
        FROM [SSD].[ssd_cla_previous_permanence]
    )
        TRUNCATE TABLE [SSD].[ssd_cla_previous_permanence];
END
ELSE
BEGIN
    CREATE TABLE [SSD].[ssd_cla_previous_permanence] (
...
INSERT INTO [SSD].[ssd_cla_previous_permanence] (
    lapp_table_id,
    lapp_person_id,
