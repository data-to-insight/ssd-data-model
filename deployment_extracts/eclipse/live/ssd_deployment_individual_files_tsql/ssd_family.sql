...
IF OBJECT_ID('[eclipseDelta].[dbo].[ssd_family]', 'U') IS NOT NULL
BEGIN
    IF EXISTS (
        SELECT 1
        FROM [eclipseDelta].[dbo].[ssd_family]
    )
        TRUNCATE TABLE [eclipseDelta].[dbo].[ssd_family];
END
ELSE
BEGIN
    CREATE TABLE [eclipseDelta].[dbo].[ssd_family] (
...
INSERT INTO [eclipseDelta].[dbo].[ssd_family] (
    fami_table_id,
    fami_family_id,
    fami_person_id
)
SELECT
    CONVERT(
        NVARCHAR(48),
        CONCAT(
            CONVERT(NVARCHAR(48), rf.GROUPID),
            CONVERT(NVARCHAR(48), rf.PERSONID)
        )
    ) AS fami_table_id,  -- metadata={"item_ref":"FAMI003A"}
