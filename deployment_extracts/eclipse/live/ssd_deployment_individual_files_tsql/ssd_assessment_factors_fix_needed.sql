...
;WITH FAPV_BASE AS (
    ...
    SELECT
        ...
        CONVERT(NVARCHAR(200), CONTROLNAME)       AS CONTROLNAME,
        CONVERT(NVARCHAR(MAX), ANSWERVALUE)       AS ANSWERVALUE,
        CONVERT(NVARCHAR(MAX), SHORTANSWERVALUE)  AS SHORTANSWERVALUE
    ...
    WHERE FAPV.DESIGNGUID = '94b3f530-a918-4f33-85c2-0ae355c9c2fd'
      ...
      AND ISNULL(CONVERT(NVARCHAR(100), FAPV.DESIGNSUBNAME), N'?')
    IN (N'Reassessment', N'Single assessment')
