...
SELECT
    ...
    CASE
        WHEN CIN_EPISODE.REFERRAL_SOURCE = 'Acquaintance'                               THEN '1B'
    ...
    JSON_BUILD_OBJECT( 
        'OUTCOME_SINGLE_ASSESSMENT_FLAG',
            CASE WHEN CIN_EPISODE.NEXT_STEP IN (
                        'Assessment',
            ...
            END,
        'OUTCOME_NFA_FLAG',
            CASE WHEN CIN_EPISODE.NEXT_STEP IN ('No further action','Signpost')
                 THEN 'Y'
                 ELSE 'N'
            END,
        'OUTCOME_STRATEGY_DISCUSSION_FLAG',
            CASE WHEN CIN_EPISODE.NEXT_STEP = 'Strategy Discussion/Meeting'
                 THEN 'Y'
                 ELSE 'N'
            END,
        'OUTCOME_CLA_REQUEST_FLAG', 'N',
        'OUTCOME_NON_AGENCY_ADOPTION_FLAG',
            CASE WHEN CIN_EPISODE.NEXT_STEP = 'Adoption or Special Guardianship support'
                 THEN 'Y'
                 ELSE 'N'
            END,
        'OUTCOME_PRIVATE_FOSTERING_FLAG',
            CASE WHEN CIN_EPISODE.NEXT_STEP = 'Private Fostering'
                 THEN 'Y'
                 ELSE 'N'
            END,
        'OUTCOME_CP_TRANSFER_IN_FLAG',
            CASE WHEN CIN_EPISODE.NEXT_STEP = 'Transfer in child protection conference'
                 THEN 'Y'
                 ELSE 'N'
            END,
    ...
    CASE
        ...
        WHEN CIN_EPISODE.CINE_REASON_END = 'Child moved permanently from area'
             THEN 'RC5'
        WHEN CIN_EPISODE.CINE_REASON_END IN (
        ...
        WHEN CIN_EPISODE.CINE_REASON_END = 'Case closed after assessment, no further action'
             THEN 'RC8'
        WHEN CIN_EPISODE.CINE_REASON_END IN (
                 'Case closed after assessment, referred to early help',
                 'Case closed after assessment, referred to EH'
             ) THEN 'RC9'
        WHEN CIN_EPISODE.CINE_CLOSE_DATE IS NULL
             THEN ''
        ELSE 'RC7'
...
WHERE
    ...
    EXISTS (
        SELECT 1
        FROM ssd_person sp
        WHERE sp.pers_person_id = CIN_EPISODE.PERSONID
    );
