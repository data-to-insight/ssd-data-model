...
SELECT
    ...
    -- a.cina_assessment_team							AS AllocatedAssessmentTeam,
    -- a.cina_assessment_worker_id						AS AllocatedAssessmentWorker,
    dept.dept_team_name                             AS AllocatedTeam,
    pro.prof_professional_name                      AS AllocatedWorker
...
LEFT JOIN
	ssd_initial_cp_conference icpc ON s47e.s47e_s47_enquiry_id = icpc.icpc_s47_enquiry_id	
...
DB Compatibility: SQL Server 2014+|...
Version: 1.1:
            1.0: Fix incorrect source for clar_ fields, allocated TeamName not ID 300724 RH
...
(
    ...
    -- get number of previous CP Plans.  NOTE: ssd_cp_plans has details of CP Plans open in the last 6 years (threshold is defined by SSD timeframe)
    LEFT JOIN
        (
            SELECT
    ...
    -- get latest allocated Team and Worker
    LEFT JOIN
        (
            SELECT
                cp.cppl_person_id PersonID,
                cp.cppl_cp_plan_id CPRegID,
                --inv.invo_professional_team Team,  -- Has Team ID only
                dpt.dept_team_name AS Team,         -- Obtains Team Name from ssd_department
                pro.prof_professional_name WorkerName,
            ...
            LEFT JOIN -- in order to retrieve TeamName as invo_professional_team contains only ID
                ssd_department dpt ON inv.invo_professional_team = dpt.dept_team_id
...
DB Compatibility: SQL Server 2014+|...
Version: 1.1
            1.0: fixed inconsistent use of MAX(), now ROWNUM() 200624 RH
            0.9: PW/Blackpool major edits/reworked PW 030324
            0.3: Removed old obj/item naming.
...
-- Check if exists & drop
IF OBJECT_ID('AA_8_children_in_care') IS NOT NULL DROP TABLE AA_8_children_in_care;
IF OBJECT_ID('tempdb..#AA_8_children_in_care') IS NOT NULL DROP TABLE #AA_8_children_in_care;
 
 
-- CTE (assists with suppressing dup records)
;WITH CTE AS (
    SELECT
        p.pers_person_id AS ChildUniqueID,  
        CASE
        ...
        inv.Team AS AllocatedTeam,
        inv.WorkerName AS AllocatedWorker,
        ROW_NUMBER() OVER (PARTITION BY p.pers_person_id ORDER BY (SELECT NULL)) AS RowNum
    FROM
        (
            SELECT
                clapl2.PersonID,
    ...
    LEFT JOIN
        (
            ...
            WHERE
                ...
                AND COALESCE(leg.lega_legal_status, 'zzz') NOT IN ('V1', 'V3', 'V4')
    ...
    LEFT JOIN
        ...
        ON clapl.PersonID = clals.PersonID
        AND clals.Rnk = 1
    LEFT JOIN
        (
            SELECT
            ...
            WHERE
                ...
                AND rev.clar_cla_review_cancelled NOT IN ('Y', 'Yes')
        ) rev
        ON clapl.PersonID = rev.PersonID
        AND rev.Rnk = 1
    LEFT JOIN
        (
            SELECT
        ...
        ON clapl.PersonID = vis.PersonID
        AND vis.Rnk = 1
    LEFT JOIN
        (
            SELECT
                lacp.lacp_person_id PersonID,
                clap.clap_cla_placement_start_date,
        ...
        ON clapl.PersonID = perm.PersonID
        AND perm.Rnk = 1
    LEFT JOIN
         (
            SELECT
    ...
    LEFT JOIN
        ...
        ON clapl.PersonID = agglacp.clae_person_id
    LEFT JOIN
        (
            SELECT
                clap.clap_person_id PersonID,
            ...
            WHERE
                COALESCE(clap.clap_cla_placement_end_date, '99991231') >= DATEADD(MONTH, -@AA_ReportingPeriod, GETDATE())
            GROUP BY
                clap.clap_person_id, clap.clap_cla_placement_id, clap.clap_cla_placement_start_date, m.miss_table_id
        ) mis
        ON clapl.PersonID = mis.PersonID
    ...
    LEFT JOIN
        ...
        ON clapl.PersonID = rhi.PersonID
        AND rhi.Rnk = 1
    LEFT JOIN
        (
            SELECT
