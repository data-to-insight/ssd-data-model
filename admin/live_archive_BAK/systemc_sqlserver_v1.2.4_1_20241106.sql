...
VALUES 
    ...
    ('1.2.4', GETDATE(), 'legacy support for json fields #LEGACY-PRE2016 tags', 1, 'admin', 'all json field alternative sql');
...
WHERE EXISTS 
    ...
    SELECT 1 
    FROM ssd_development.ssd_person p
    WHERE TRY_CAST(p.pers_person_id AS INT) = pa.DIM_PERSON_ID -- #DtoI-1799
    );
...
-- 1. filter and store results in tmp table
SELECT 
    ffa.FACT_FORM_ID,
    ffa.ANSWER_NO,
    ffa.ANSWER
...
-- 2. concat values using FOR XML PATH
SELECT 
    FACT_FORM_ID,
    '[' + STUFF((
        SELECT ',"' + ANSWER_NO + '"' 
        FROM #ssd_TMP_PRE_assessment_factors ffa2
        WHERE ffa2.FACT_FORM_ID = ffa1.FACT_FORM_ID
        FOR XML PATH(''), TYPE
    ).value('.', 'NVARCHAR(MAX)'), 1, 1, '') + ']' AS cinf_assessment_factors_json
INTO #Final_Concat
FROM #ssd_TMP_PRE_assessment_factors ffa1
GROUP BY FACT_FORM_ID;

-- 3. join and insert with batching
...
BEGIN
    ...
    INSERT INTO ssd_development.ssd_assessment_factors (cinf_table_id, cinf_assessment_id, cinf_assessment_factors_json)
    SELECT 
        fsa.EXTERNAL_ID, 
        fsa.FACT_FORM_ID, 
        batch.cinf_assessment_factors_json
    FROM 
        HDM.Child_Social.FACT_SINGLE_ASSESSMENT fsa
    JOIN Batch batch ON batch.FACT_FORM_ID = fsa.FACT_FORM_ID;
...
-- -- Opt1: (alternative implementation for backward compatibility)
