...
BEGIN
    ...
    -- DECLARE @schema_name NVARCHAR(128) = N'';    -- Set your schema name here. Leave empty for default behaviour
...
Status: [R]elease
Remarks: PK currently disabled due to (known) data conflict
        The duplicate key value is (12833). 
        CP Plans in the Warehouse has been logged with incorrect FACT_INITIAL_CP_CONFERENCE_ID and FACT_REFERRAL_ID. 
        Instead of the correct ones, these fields are duplicated with an earlier CP Plan on the same person.
        Equates to == 2 CP Plans apparently resulting from the same ICPC.
...
CREATE TABLE ssd_initial_cp_conference (
    icpc_icpc_id                NVARCHAR(48),               -- metadata={"item_ref":"ICPC001A"} --  PRIMARY KEY (See Remarks)
...
BEGIN
    -- -- add constraint(s)
    -- ALTER TABLE ssd_cla_convictions ADD CONSTRAINT FK_ssd_clac_to_clae 
...
SELECT
    ...
    fi.FACT_WORKER_HISTORY_DEPARTMENT_ID          AS invo_professional_team, -- replacing the above [TESTING] #DtoI-1762
...
BEGIN
    ...
    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_prep_person_id                ON ssd_pre_proceedings (prep_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_prep_pre_pro_decision_date    ON ssd_pre_proceedings (prep_pre_pro_decision_date);
    CREATE NONCLUSTERED INDEX idx_ssd_prep_legal_gateway_outcome    ON ssd_pre_proceedings (prep_legal_gateway_outcome);
END
