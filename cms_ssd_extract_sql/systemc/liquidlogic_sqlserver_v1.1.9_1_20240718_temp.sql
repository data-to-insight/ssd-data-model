...
BEGIN
    ...
    /* START drop all #ssd_ schema constraints */
    ...
    /* END Drop all #ssd_ schema constraints */
END
...
IF OBJECT_ID('ssd_development.ssd_version_log', 'U') IS NOT NULL DROP TABLE #ssd_ssd_version_log;
IF OBJECT_ID('tempdb..#ssd_version_log', 'U') IS NOT NULL DROP TABLE #ssd_version_log;

-- create versioning information object
CREATE TABLE #ssd_ssd_version_log (
...
UPDATE #ssd_ssd_version_log SET is_current = 0 WHERE is_current = 1;


-- insert & update current version (using MAJOR.MINOR.PATCH)
INSERT INTO #ssd_ssd_version_log 
...
INSERT INTO #ssd_ssd_version_log (version_number, release_date, description, is_current, created_by, impact_description)
VALUES 
...
-- check exists & drop
IF OBJECT_ID('ssd_development.ssd_person') IS NOT NULL DROP TABLE #ssd_ssd_person;
IF OBJECT_ID('tempdb..#ssd_person') IS NOT NULL DROP TABLE #ssd_person;


-- Create structure
CREATE TABLE #ssd_ssd_person (
...
BEGIN
    ...
    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_person_pers_dob               ON #ssd_ssd_person(pers_dob);
    CREATE NONCLUSTERED INDEX idx_ssd_person_pers_common_child_id   ON #ssd_ssd_person(pers_common_child_id);
    CREATE NONCLUSTERED INDEX idx_ssd_person_ethnicity_gender       ON #ssd_ssd_person(pers_ethnicity, pers_gender);
END
...
-- check exists & drop
IF OBJECT_ID('ssd_development.ssd_family') IS NOT NULL DROP TABLE #ssd_ssd_family;
IF OBJECT_ID('tempdb..#ssd_family') IS NOT NULL DROP TABLE #ssd_family;


-- Create structure
CREATE TABLE #ssd_ssd_family (
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_family ADD CONSTRAINT FK_family_person
    FOREIGN KEY (fami_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_family_person_id              ON #ssd_ssd_family(fami_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_family_fami_family_id     ON #ssd_ssd_family(fami_family_id);
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_address') IS NOT NULL DROP TABLE #ssd_ssd_address;
IF OBJECT_ID('tempdb..#ssd_address') IS NOT NULL DROP TABLE #ssd_address;


-- Create structure
CREATE TABLE #ssd_ssd_address (
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_address ADD CONSTRAINT FK_address_person
    FOREIGN KEY (addr_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_address_person        ON #ssd_ssd_address(addr_person_id);
    CREATE NONCLUSTERED INDEX idx_address_start         ON #ssd_ssd_address(addr_address_start_date);
    CREATE NONCLUSTERED INDEX idx_address_end           ON #ssd_ssd_address(addr_address_end_date);
    CREATE NONCLUSTERED INDEX idx_ssd_address_postcode  ON #ssd_ssd_address(addr_address_postcode);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_disability') IS NOT NULL DROP TABLE #ssd_ssd_disability;
IF OBJECT_ID('tempdb..#ssd_disability') IS NOT NULL DROP TABLE #ssd_disability;

-- Create the structure
CREATE TABLE #ssd_ssd_disability
(
...
-- Insert data
INSERT INTO #ssd_ssd_disability (
    disa_table_id,  
    disa_person_id, 
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_disability ADD CONSTRAINT FK_disability_person 
    FOREIGN KEY (disa_person_id) REFERENCES #ssd_ssd_person(pers_person_id);
        
    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_disability_person_id ON #ssd_ssd_disability(disa_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_disability_code ON #ssd_ssd_disability(disa_disability_code);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_immigration_status') IS NOT NULL DROP TABLE #ssd_ssd_immigration_status;
IF OBJECT_ID('tempdb..#ssd_immigration_status') IS NOT NULL DROP TABLE #ssd_immigration_status;


-- Create structure
CREATE TABLE #ssd_ssd_immigration_status (
...
-- insert data
INSERT INTO #ssd_ssd_immigration_status (
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_immigration_status ADD CONSTRAINT FK_immigration_status_person
    FOREIGN KEY (immi_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_immigration_status_immi_person_id ON #ssd_ssd_immigration_status(immi_person_id);
    CREATE NONCLUSTERED INDEX idx_immigration_status_start          ON #ssd_ssd_immigration_status(immi_immigration_status_start_date);
    CREATE NONCLUSTERED INDEX idx_immigration_status_end            ON #ssd_ssd_immigration_status(immi_immigration_status_end_date);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_mother', 'U') IS NOT NULL DROP TABLE #ssd_ssd_mother;
IF OBJECT_ID('tempdb..#ssd_mother') IS NOT NULL DROP TABLE #ssd_mother;


-- Create structure
CREATE TABLE #ssd_ssd_mother (
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_mother ADD CONSTRAINT FK_moth_to_person 
    FOREIGN KEY (moth_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- -- [TESTING] deployment issues remain
    -- ALTER TABLE #ssd_ssd_mother ADD CONSTRAINT FK_child_to_person 
    -- FOREIGN KEY (moth_childs_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- -- [TESTING] Comment this out for ESCC until further notice
    -- ALTER TABLE #ssd_ssd_mother ADD CONSTRAINT CHK_no_self_parenting -- Ensure person cannot be their own mother
    -- CHECK (moth_person_id <> moth_childs_person_id);


    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_mother_moth_person_id ON #ssd_ssd_mother(moth_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_mother_childs_person_id ON #ssd_ssd_mother(moth_childs_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_mother_childs_dob ON #ssd_ssd_mother(moth_childs_dob);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_legal_status') IS NOT NULL DROP TABLE #ssd_ssd_legal_status;
IF OBJECT_ID('tempdb..#ssd_legal_status') IS NOT NULL DROP TABLE #ssd_legal_status;

-- Create structure
CREATE TABLE #ssd_ssd_legal_status (
...
-- Insert data
INSERT INTO #ssd_ssd_legal_status (
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_legal_status ADD CONSTRAINT FK_legal_status_person
    FOREIGN KEY (lega_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_legal_status_lega_person_id   ON #ssd_ssd_legal_status(lega_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_legal_status                  ON #ssd_ssd_legal_status(lega_legal_status);
    CREATE NONCLUSTERED INDEX idx_ssd_legal_status_start            ON #ssd_ssd_legal_status(lega_legal_status_start_date);
    CREATE NONCLUSTERED INDEX idx_ssd_legal_status_end              ON #ssd_ssd_legal_status(lega_legal_status_end_date);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_contacts') IS NOT NULL DROP TABLE #ssd_ssd_contacts;
IF OBJECT_ID('tempdb..#ssd_contacts') IS NOT NULL DROP TABLE #ssd_contacts;


-- Create structure
CREATE TABLE #ssd_ssd_contacts (
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_contacts ADD CONSTRAINT FK_contact_person 
    FOREIGN KEY (cont_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_contact_person_id     ON #ssd_ssd_contacts(cont_person_id);
    CREATE NONCLUSTERED INDEX idx_contact_date          ON #ssd_ssd_contacts(cont_contact_date);
    CREATE NONCLUSTERED INDEX idx_contact_source_code   ON #ssd_ssd_contacts(cont_contact_source_code);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_early_help_episodes') IS NOT NULL DROP TABLE #ssd_ssd_early_help_episodes;
IF OBJECT_ID('tempdb..#ssd_early_help_episodes') IS NOT NULL DROP TABLE #ssd_early_help_episodes;


-- Create structure
CREATE TABLE #ssd_ssd_early_help_episodes (
...
-- Insert data
INSERT INTO #ssd_ssd_early_help_episodes (
    earl_episode_id,
    earl_person_id,
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_early_help_episodes ADD CONSTRAINT FK_earl_to_person 
    FOREIGN KEY (earl_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_early_help_episodes_person_id ON #ssd_ssd_early_help_episodes(earl_person_id);
    CREATE NONCLUSTERED INDEX idx_early_help_start_date             ON #ssd_ssd_early_help_episodes(earl_episode_start_date);
    CREATE NONCLUSTERED INDEX idx_early_help_end_date               ON #ssd_ssd_early_help_episodes(earl_episode_end_date);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_cin_episodes') IS NOT NULL DROP TABLE #ssd_ssd_cin_episodes;
IF OBJECT_ID('tempdb..#ssd_cin_episodes') IS NOT NULL DROP TABLE #ssd_cin_episodes;

-- Create structure
CREATE TABLE #ssd_ssd_cin_episodes
(
...
-- Insert data
INSERT INTO #ssd_ssd_cin_episodes
(
    cine_referral_id,
    cine_person_id,
    cine_referral_date,
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_cin_episodes ADD CONSTRAINT FK_ssd_cin_episodes_to_person 
    FOREIGN KEY (cine_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_cin_episodes_person_id    ON #ssd_ssd_cin_episodes(cine_person_id);
    CREATE NONCLUSTERED INDEX idx_cin_referral_date             ON #ssd_ssd_cin_episodes(cine_referral_date);
    CREATE NONCLUSTERED INDEX idx_cin_close_date                ON #ssd_ssd_cin_episodes(cine_close_date);
END
...
-- Check if exists, & drop 
IF OBJECT_ID('ssd_development.ssd_cin_assessments') IS NOT NULL DROP TABLE #ssd_ssd_cin_assessments;
IF OBJECT_ID('tempdb..#ssd_cin_assessments') IS NOT NULL DROP TABLE #ssd_cin_assessments;


-- Create structure
CREATE TABLE #ssd_ssd_cin_assessments
(
...
-- Insert data
INSERT INTO #ssd_ssd_cin_assessments
(
    cina_assessment_id,
    cina_person_id,
    cina_referral_id,
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_cin_assessments ADD CONSTRAINT FK_ssd_cin_assessments_to_person 
    FOREIGN KEY (cina_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_cin_assessments_person_id ON #ssd_ssd_cin_assessments(cina_person_id);
    CREATE NONCLUSTERED INDEX idx_cina_assessment_start_date    ON #ssd_ssd_cin_assessments(cina_assessment_start_date);
    CREATE NONCLUSTERED INDEX idx_cina_assessment_auth_date     ON #ssd_ssd_cin_assessments(cina_assessment_auth_date);
    CREATE NONCLUSTERED INDEX idx_cina_referral_id              ON #ssd_ssd_cin_assessments(cina_referral_id);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_assessment_factors') IS NOT NULL DROP TABLE #ssd_ssd_assessment_factors;
...
-- Create structure
CREATE TABLE #ssd_ssd_assessment_factors (
...
INSERT INTO #ssd_ssd_assessment_factors (
               cinf_table_id, 
               cinf_assessment_id, 
...
WHERE 
    fsa.EXTERNAL_ID <> -1
    AND fsa.FACT_FORM_ID IN (SELECT cina_assessment_id FROM #ssd_ssd_cin_assessments);
...
BEGIN
    ...
    ALTER TABLE #ssd_ssd_assessment_factors ADD CONSTRAINT FK_cinf_assessment_id
    FOREIGN KEY (cinf_assessment_id) REFERENCES #ssd_ssd_cin_assessments(cina_assessment_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_cinf_assessment_id ON #ssd_ssd_assessment_factors(cinf_assessment_id);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_cin_plans', 'U') IS NOT NULL DROP TABLE #ssd_ssd_cin_plans;
IF OBJECT_ID('tempdb..#ssd_cin_plans', 'U') IS NOT NULL DROP TABLE #ssd_cin_plans;


-- Create structure
CREATE TABLE #ssd_ssd_cin_plans (
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_cin_plans ADD CONSTRAINT FK_cinp_to_person 
    FOREIGN KEY (cinp_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_cin_plans_person_id   ON #ssd_ssd_cin_plans(cinp_person_id);
    CREATE NONCLUSTERED INDEX idx_cinp_cin_plan_start_date  ON #ssd_ssd_cin_plans(cinp_cin_plan_start_date);
    CREATE NONCLUSTERED INDEX idx_cinp_cin_plan_end_date    ON #ssd_ssd_cin_plans(cinp_cin_plan_end_date);
    CREATE NONCLUSTERED INDEX idx_cinp_referral_id          ON #ssd_ssd_cin_plans(cinp_referral_id);
END
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.ssd_cin_visits') IS NOT NULL DROP TABLE #ssd_ssd_cin_visits;
IF OBJECT_ID('tempdb..#ssd_cin_visits') IS NOT NULL DROP TABLE #ssd_cin_visits;
 
-- Create structure
CREATE TABLE #ssd_ssd_cin_visits
(
...
-- Insert data
INSERT INTO #ssd_ssd_cin_visits
(
    cinv_cin_visit_id,                  
    cinv_person_id,
    cinv_cin_visit_date,
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_cin_visits ADD CONSTRAINT FK_ssd_cin_visits_to_person
    FOREIGN KEY (cinv_person_id) REFERENCES #ssd_ssd_person(pers_person_id);
    
    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_cinv_person_id        ON #ssd_ssd_cin_visits(cinv_person_id);
    CREATE NONCLUSTERED INDEX idx_cinv_cin_visit_date   ON #ssd_ssd_cin_visits(cinv_cin_visit_date);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_s47_enquiry') IS NOT NULL DROP TABLE #ssd_ssd_s47_enquiry;
IF OBJECT_ID('tempdb..#ssd_s47_enquiry') IS NOT NULL DROP TABLE #ssd_s47_enquiry;

-- Create structure 
CREATE TABLE #ssd_ssd_s47_enquiry (
...
-- insert data
INSERT INTO #ssd_ssd_s47_enquiry(
    s47e_s47_enquiry_id,
    s47e_referral_id,
    s47e_person_id,
    s47e_s47_start_date,
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_s47_enquiry ADD CONSTRAINT FK_s47_person
    FOREIGN KEY (s47e_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_s47_enquiry_person_id     ON #ssd_ssd_s47_enquiry(s47e_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_s47_enquiry_start_date    ON #ssd_ssd_s47_enquiry(s47e_s47_start_date);
    CREATE NONCLUSTERED INDEX idx_ssd_s47_enquiry_end_date      ON #ssd_ssd_s47_enquiry(s47e_s47_end_date);
    CREATE NONCLUSTERED INDEX idx_ssd_s47_enquiry_referral_id   ON #ssd_ssd_s47_enquiry(s47e_referral_id);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_initial_cp_conference') IS NOT NULL DROP TABLE #ssd_ssd_initial_cp_conference;
IF OBJECT_ID('tempdb..#ssd_initial_cp_conference') IS NOT NULL DROP TABLE #ssd_initial_cp_conference;
 

-- Create structure
CREATE TABLE #ssd_ssd_initial_cp_conference (
...
-- insert data
INSERT INTO #ssd_ssd_initial_cp_conference(
    icpc_icpc_id,
...
IF @Run_SSD_As_Temporary_Tables = 0
BEGIN
    ALTER TABLE #ssd_ssd_initial_cp_conference ADD CONSTRAINT FK_icpc_person_id
    FOREIGN KEY (icpc_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- -- [TESTING] #DtoI-1769 - failing at 160724 RH
    -- ALTER TABLE #ssd_ssd_initial_cp_conference ADD CONSTRAINT FK_icpc_referral_id
    -- FOREIGN KEY (icpc_referral_id) REFERENCES #ssd_ssd_cin_episodes(cine_referral_id);


    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_icpc_person_id        ON #ssd_ssd_initial_cp_conference(icpc_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_icpc_s47_enquiry_id   ON #ssd_ssd_initial_cp_conference(icpc_s47_enquiry_id);
    CREATE NONCLUSTERED INDEX idx_ssd_icpc_referral_id      ON #ssd_ssd_initial_cp_conference(icpc_referral_id);
    CREATE NONCLUSTERED INDEX idx_ssd_icpc_icpc_date        ON #ssd_ssd_initial_cp_conference(icpc_icpc_date);
END
...
-- Check if exists & drop 
IF OBJECT_ID('ssd_development.ssd_cp_plans') IS NOT NULL DROP TABLE #ssd_ssd_cp_plans;
IF OBJECT_ID('tempdb..#ssd_cp_plans') IS NOT NULL DROP TABLE #ssd_cp_plans;

-- Create structure
CREATE TABLE #ssd_ssd_cp_plans (
...
BEGIN
    -- -- Add constraint(s)
    -- ALTER TABLE #ssd_ssd_cp_plans ADD CONSTRAINT FK_cppl_person_id
    -- FOREIGN KEY (cppl_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- ALTER TABLE #ssd_ssd_cp_plans ADD CONSTRAINT FK_cppl_icpc_id
    -- FOREIGN KEY (cppl_icpc_id) REFERENCES #ssd_ssd_initial_cp_conference(icpc_icpc_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_cp_plans_person_id ON #ssd_ssd_cp_plans(cppl_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_cp_plans_icpc_id ON #ssd_ssd_cp_plans(cppl_icpc_id);
    CREATE NONCLUSTERED INDEX idx_ssd_cp_plans_referral_id ON #ssd_ssd_cp_plans(cppl_referral_id);
    CREATE NONCLUSTERED INDEX idx_ssd_cp_plans_start_date ON #ssd_ssd_cp_plans(cppl_cp_plan_start_date);
    CREATE NONCLUSTERED INDEX idx_ssd_cp_plans_end_date ON #ssd_ssd_cp_plans(cppl_cp_plan_end_date);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_cp_visits') IS NOT NULL DROP TABLE #ssd_ssd_cp_visits;
IF OBJECT_ID('tempdb..#ssd_cp_visits') IS NOT NULL DROP TABLE #ssd_cp_visits;
  
 
-- Create structure
CREATE TABLE #ssd_ssd_cp_visits (
...
BEGIN
    -- -- Add constraint(s)
    -- ALTER TABLE #ssd_ssd_cp_visits ADD CONSTRAINT FK_cppv_to_cppl
    -- FOREIGN KEY (cppv_cp_plan_id) REFERENCES #ssd_ssd_cp_plans(cppl_cp_plan_id);
    ...
    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_cppv_person_id        ON #ssd_ssd_cp_visits(cppv_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_cppv_cp_plan_id       ON #ssd_ssd_cp_visits(cppv_cp_plan_id);
    CREATE NONCLUSTERED INDEX idx_ssd_cppv_cp_visit_date    ON #ssd_ssd_cp_visits(cppv_cp_visit_date);
END
...
IF OBJECT_ID('ssd_development.ssd_cp_reviews') IS NOT NULL DROP TABLE #ssd_ssd_cp_reviews;
IF OBJECT_ID('tempdb..#ssd_cp_reviews') IS NOT NULL DROP TABLE #ssd_cp_reviews;
  
 
-- Create structure
CREATE TABLE #ssd_ssd_cp_reviews
(
...
-- Insert data
INSERT INTO #ssd_ssd_cp_reviews
(
    cppr_cp_review_id,
    cppr_cp_plan_id,
    cppr_person_id,
...
BEGIN
    -- -- Add constraint(s)
    -- ALTER TABLE #ssd_ssd_cp_reviews ADD CONSTRAINT FK_ssd_cp_reviews_to_cp_plans 
    -- FOREIGN KEY (cppr_cp_plan_id) REFERENCES #ssd_ssd_cp_plans(cppl_cp_plan_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_cppr_person_id ON #ssd_ssd_cp_reviews(cppr_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_cppr_cp_plan_id ON #ssd_ssd_cp_reviews(cppr_cp_plan_id);
    CREATE NONCLUSTERED INDEX idx_ssd_cppr_cp_review_due ON #ssd_ssd_cp_reviews(cppr_cp_review_due);
    CREATE NONCLUSTERED INDEX idx_ssd_cppr_cp_review_date ON #ssd_ssd_cp_reviews(cppr_cp_review_date);
    CREATE NONCLUSTERED INDEX idx_ssd_cppr_cp_review_meeting_id ON #ssd_ssd_cp_reviews(cppr_cp_review_meeting_id);
END
...
-- Check if table exists, & drop
IF OBJECT_ID('ssd_development.ssd_cla_episodes') IS NOT NULL DROP TABLE #ssd_ssd_cla_episodes;
IF OBJECT_ID('tempdb..#ssd_cla_episodes') IS NOT NULL DROP TABLE #ssd_cla_episodes;

 
-- Create structure
CREATE TABLE #ssd_ssd_cla_episodes (
...
WITH FilteredData AS (
    ...
    WHERE
        ...
        CAST(fce.DIM_PERSON_ID AS NVARCHAR(48)) IN (SELECT pers_person_id FROM #ssd_ssd_person)
...
-- Insert data
INSERT INTO #ssd_ssd_cla_episodes (
    clae_cla_episode_id,
    clae_person_id,
...
-- )
-- -- WHERE
-- --     fce.DIM_PERSON_ID IN (SELECT pers_person_id FROM #ssd_ssd_person)
...
-- -- [TESTING]
-- SELECT DISTINCT clae_person_id FROM #ssd_ssd_cla_episodes WHERE clae_person_id NOT IN (SELECT pers_person_id FROM #ssd_ssd_person);


IF @Run_SSD_As_Temporary_Tables = 0
BEGIN
    -- Add constraint(s) #DtoI-1769
    ALTER TABLE #ssd_ssd_cla_episodes ADD CONSTRAINT FK_clae_to_person 
    FOREIGN KEY (clae_person_id) REFERENCES #ssd_ssd_person (pers_person_id);

    -- ALTER TABLE #ssd_ssd_cla_episodes ADD CONSTRAINT FK_clae_cla_placement_id
    -- FOREIGN KEY (clae_cla_placement_id) REFERENCES #ssd_ssd_cla_placements (clap_cla_placement_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_clae_person_id ON #ssd_ssd_cla_episodes(clae_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_clae_episode_start_date ON #ssd_ssd_cla_episodes(clae_cla_episode_start_date);
    CREATE NONCLUSTERED INDEX idx_ssd_clae_episode_ceased ON #ssd_ssd_cla_episodes(clae_cla_episode_ceased);
    CREATE NONCLUSTERED INDEX idx_ssd_clae_referral_id ON #ssd_ssd_cla_episodes(clae_referral_id);
    CREATE NONCLUSTERED INDEX idx_ssd_clae_cla_last_iro_contact_date ON #ssd_ssd_cla_episodes(clae_cla_last_iro_contact_date);
    CREATE NONCLUSTERED INDEX idx_clae_cla_placement_id ON #ssd_ssd_cla_episodes(clae_cla_placement_id);
END
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.ssd_cla_convictions', 'U') IS NOT NULL DROP TABLE #ssd_ssd_cla_convictions;
IF OBJECT_ID('tempdb..#ssd_cla_convictions', 'U') IS NOT NULL DROP TABLE #ssd_cla_convictions;


-- create structure
CREATE TABLE #ssd_ssd_cla_convictions (
...
-- insert data
INSERT INTO #ssd_ssd_cla_convictions (
...
BEGIN
    -- -- add constraint(s)
    -- ALTER TABLE #ssd_ssd_cla_convictions ADD CONSTRAINT FK_clac_to_clae 
    -- FOREIGN KEY (clac_person_id) REFERENCES #ssd_ssd_cla_episodes(clae_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_clac_person_id ON #ssd_ssd_cla_convictions(clac_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_clac_conviction_date ON #ssd_ssd_cla_convictions(clac_cla_conviction_date);
END
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.ssd_cla_health', 'U') IS NOT NULL DROP TABLE #ssd_ssd_cla_health;
IF OBJECT_ID('tempdb..#ssd_cla_health', 'U') IS NOT NULL DROP TABLE #ssd_cla_health;

-- create structure
CREATE TABLE #ssd_ssd_cla_health (
...
-- insert data
INSERT INTO #ssd_ssd_cla_health (
...
BEGIN
    -- -- add constraint(s)
    -- ALTER TABLE #ssd_ssd_cla_health ADD CONSTRAINT FK_clah_to_clae 
    -- FOREIGN KEY (clah_person_id) REFERENCES #ssd_ssd_cla_episodes(clae_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_clah_person_id ON #ssd_ssd_cla_health (clah_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_clah_health_check_date ON #ssd_ssd_cla_health(clah_health_check_date);
    CREATE NONCLUSTERED INDEX idx_ssd_clah_health_check_status ON #ssd_ssd_cla_health(clah_health_check_status);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_cla_immunisations') IS NOT NULL DROP TABLE #ssd_ssd_cla_immunisations;
IF OBJECT_ID('tempdb..#ssd_cla_immunisations') IS NOT NULL DROP TABLE #ssd_cla_immunisations;

-- Create structure
CREATE TABLE #ssd_ssd_cla_immunisations (
...
INSERT INTO #ssd_ssd_cla_immunisations (
    clai_person_id,
...
BEGIN
    -- -- add constraint(s)
    -- ALTER TABLE #ssd_ssd_cla_immunisations ADD CONSTRAINT FK_ssd_cla_immunisations_person
    -- FOREIGN KEY (clai_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_clai_person_id ON #ssd_ssd_cla_immunisations(clai_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_clai_immunisations_status ON #ssd_ssd_cla_immunisations(clai_immunisations_status);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_cla_substance_misuse') IS NOT NULL DROP TABLE #ssd_ssd_cla_substance_misuse;
IF OBJECT_ID('tempdb..#ssd_cla_substance_misuse') IS NOT NULL DROP TABLE #ssd_cla_substance_misuse;

-- Create structure 
CREATE TABLE #ssd_ssd_cla_substance_misuse (
...
-- Insert data
INSERT INTO #ssd_ssd_cla_substance_misuse (
...
BEGIN
    -- -- Add constraint(s)
    -- ALTER TABLE #ssd_ssd_cla_substance_misuse ADD CONSTRAINT FK_ssd_cla_substance_misuse_clas_person_id 
    -- FOREIGN KEY (clas_person_id) REFERENCES #ssd_ssd_cla_episodes (clae_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_clas_person_id ON #ssd_ssd_cla_substance_misuse (clas_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_clas_substance_misuse_date ON #ssd_ssd_cla_substance_misuse(clas_substance_misuse_date);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_cla_placement', 'U') IS NOT NULL DROP TABLE #ssd_ssd_cla_placement;
IF OBJECT_ID('tempdb..#ssd_cla_placement', 'U') IS NOT NULL DROP TABLE #ssd_cla_placement;
  
-- Create structure
CREATE TABLE #ssd_ssd_cla_placement (
...
-- Insert data
INSERT INTO #ssd_ssd_cla_placement (
...
BEGIN
    -- -- Add constraint(s)
    -- ALTER TABLE #ssd_ssd_cla_placement ADD CONSTRAINT FK_clap_to_clae 
    -- FOREIGN KEY (clap_cla_id) REFERENCES #ssd_ssd_cla_episodes(clae_cla_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_clap_cla_placement_urn ON #ssd_ssd_cla_placement (clap_cla_placement_urn);
    CREATE NONCLUSTERED INDEX idx_ssd_clap_cla_id ON #ssd_ssd_cla_placement(clap_cla_id);
    CREATE NONCLUSTERED INDEX idx_ssd_clap_placement_start_date ON #ssd_ssd_cla_placement(clap_cla_placement_start_date);
    CREATE NONCLUSTERED INDEX idx_ssd_clap_placement_end_date ON #ssd_ssd_cla_placement(clap_cla_placement_end_date);
    CREATE NONCLUSTERED INDEX idx_ssd_clap_placement_postcode ON #ssd_ssd_cla_placement(clap_cla_placement_postcode);
    CREATE NONCLUSTERED INDEX idx_ssd_clap_placement_type ON #ssd_ssd_cla_placement(clap_cla_placement_type);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_cla_reviews', 'U') IS NOT NULL DROP TABLE #ssd_ssd_cla_reviews;
IF OBJECT_ID('tempdb..#ssd_cla_reviews', 'U') IS NOT NULL DROP TABLE #ssd_cla_reviews;
  
-- Create structure
CREATE TABLE #ssd_ssd_cla_reviews (
...
-- Insert data
INSERT INTO #ssd_ssd_cla_reviews (
    clar_cla_review_id,
    clar_cla_id,
...
BEGIN
    -- -- Add constraint(s)
    -- ALTER TABLE #ssd_ssd_cla_reviews ADD CONSTRAINT FK_clar_to_clae 
    -- FOREIGN KEY (clar_cla_id) REFERENCES #ssd_ssd_cla_episodes(clae_cla_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_clar_cla_id ON #ssd_ssd_cla_reviews(clar_cla_id);
    CREATE NONCLUSTERED INDEX idx_ssd_clar_review_due_date ON #ssd_ssd_cla_reviews(clar_cla_review_due_date);
    CREATE NONCLUSTERED INDEX idx_ssd_clar_review_date ON #ssd_ssd_cla_reviews(clar_cla_review_date);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_cla_previous_permanence') IS NOT NULL DROP TABLE #ssd_ssd_cla_previous_permanence;
...
-- Create structure
CREATE TABLE #ssd_ssd_cla_previous_permanence (
...
-- Insert data
INSERT INTO #ssd_ssd_cla_previous_permanence (
               lapp_table_id,
               lapp_person_id,
...
BEGIN
    -- -- Add constraint(s)
    -- ALTER TABLE #ssd_ssd_cla_previous_permanence ADD CONSTRAINT FK_lapp_person_id
    -- FOREIGN KEY (lapp_person_id) REFERENCES #ssd_ssd_cla_episodes(clae_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_lapp_person_id ON #ssd_ssd_cla_previous_permanence(lapp_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_lapp_previous_permanence_option ON #ssd_ssd_cla_previous_permanence(lapp_previous_permanence_option);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_cla_care_plan', 'U') IS NOT NULL DROP TABLE #ssd_ssd_cla_care_plan;
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_pre_cla_care_plan', 'U') IS NOT NULL DROP TABLE #ssd_ssd_pre_cla_care_plan;
IF OBJECT_ID('tempdb..#ssd_pre_cla_care_plan', 'U') IS NOT NULL DROP TABLE #ssd_pre_cla_care_plan;

CREATE TABLE #ssd_ssd_pre_cla_care_plan (
...
INSERT INTO #ssd_ssd_pre_cla_care_plan (
    FACT_FORM_ID,
    DIM_PERSON_ID,
    ANSWER_NO,
    ANSWER,
...
-- Create structure
CREATE TABLE #ssd_ssd_cla_care_plan (
...
-- Insert data
INSERT INTO #ssd_ssd_cla_care_plan (
    lacp_table_id,
    lacp_person_id,
...
SELECT
    ...
    (
        ...
        FROM
            ...
            #ssd_ssd_pre_cla_care_plan tmp_cpl
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_cla_care_plan ADD CONSTRAINT FK_lacp_person_id
    FOREIGN KEY (lacp_person_id) REFERENCES #ssd_ssd_person(pers_person_id);
    
    -- create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_lacp_person_id ON #ssd_ssd_cla_care_plan(lacp_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_lacp_care_plan_start_date ON #ssd_ssd_cla_care_plan(lacp_cla_care_plan_start_date);
    CREATE NONCLUSTERED INDEX idx_ssd_lacp_care_plan_end_date ON #ssd_ssd_cla_care_plan(lacp_cla_care_plan_end_date);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_cla_visits', 'U') IS NOT NULL DROP TABLE #ssd_ssd_cla_visits;
IF OBJECT_ID('tempdb..#ssd_cla_visits', 'U') IS NOT NULL DROP TABLE #ssd_cla_visits;


-- Create structure
CREATE TABLE #ssd_ssd_cla_visits (
...
-- Insert data
INSERT INTO #ssd_ssd_cla_visits (
    clav_cla_visit_id,
    clav_cla_id,
    clav_person_id,
    clav_cla_visit_date,
...
BEGIN
    -- -- Add constraint(s)
    -- ALTER TABLE #ssd_ssd_cla_visits ADD CONSTRAINT FK_clav_person_id
    -- FOREIGN KEY (clav_person_id) REFERENCES #ssd_ssd_cla_episodes(clae_person_id);

    -- create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_clav_person_id ON #ssd_ssd_cla_visits(clav_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_clav_visit_date ON #ssd_ssd_cla_visits(clav_cla_visit_date);
    CREATE NONCLUSTERED INDEX idx_ssd_clav_cla_id ON #ssd_ssd_cla_visits(clav_cla_id);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_sdq_scores', 'U') IS NOT NULL DROP TABLE #ssd_ssd_sdq_scores;
IF OBJECT_ID('tempdb..#ssd_sdq_scores', 'U') IS NOT NULL DROP TABLE #ssd_sdq_scores;
 
 
-- Create structure
CREATE TABLE #ssd_ssd_sdq_scores (
...
-- insert data
INSERT INTO #ssd_ssd_sdq_scores (
    csdq_table_id, 
    csdq_person_id, 
...
BEGIN
    -- -- Add constraint(s)
    -- ALTER TABLE #ssd_ssd_sdq_scores ADD CONSTRAINT FK_csdq_person_id
    -- FOREIGN KEY (csdq_person_id) REFERENCES #ssd_ssd_person(pers_person_id);
    ...
    -- create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_csdq_person_id ON #ssd_ssd_sdq_scores(csdq_person_id);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_missing', 'U') IS NOT NULL DROP TABLE #ssd_ssd_missing;
IF OBJECT_ID('tempdb..#ssd_missing', 'U') IS NOT NULL DROP TABLE #ssd_missing;

-- Create structure
CREATE TABLE #ssd_ssd_missing (
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_missing ADD CONSTRAINT FK_missing_to_person
    FOREIGN KEY (miss_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_miss_person_id        ON #ssd_ssd_missing(miss_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_miss_episode_start    ON #ssd_ssd_missing(miss_missing_episode_start_date);
    CREATE NONCLUSTERED INDEX idx_ssd_miss_episode_end      ON #ssd_ssd_missing(miss_missing_episode_end_date);
    CREATE NONCLUSTERED INDEX idx_ssd_miss_rhi_offered      ON #ssd_ssd_missing(miss_missing_rhi_offered);
    CREATE NONCLUSTERED INDEX idx_ssd_miss_rhi_accepted     ON #ssd_ssd_missing(miss_missing_rhi_accepted);
END
...
Remarks:    Dev: Note that <multiple> refs to ssd_person need changing when porting code to tempdb.. versions.
            Dev: Ensure index on ssd_person.pers_person_id is intact to ensure performance on <FROM #ssd_ssd_person> references in the CTEs(added for performance)
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_care_leavers', 'U') IS NOT NULL DROP TABLE #ssd_ssd_care_leavers;
IF OBJECT_ID('tempdb..#ssd_care_leavers', 'U') IS NOT NULL DROP TABLE #ssd_care_leavers;
 
 
-- Create structure
CREATE TABLE #ssd_ssd_care_leavers
(
...
-- Insert data
INSERT INTO #ssd_ssd_care_leavers
(
    clea_table_id,
    clea_person_id,
...
BEGIN
    -- -- Add constraint(s)
    -- ALTER TABLE #ssd_ssd_care_leavers ADD CONSTRAINT FK_care_leavers_person
    -- FOREIGN KEY (clea_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_clea_person_id                        ON #ssd_ssd_care_leavers(clea_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_clea_care_leaver_latest_contact   ON #ssd_ssd_care_leavers(clea_care_leaver_latest_contact);
    CREATE NONCLUSTERED INDEX idx_ssd_clea_pathway_plan_review_date     ON #ssd_ssd_care_leavers(clea_pathway_plan_review_date);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_permanence', 'U') IS NOT NULL DROP TABLE #ssd_ssd_permanence;
IF OBJECT_ID('tempdb..#ssd_permanence', 'U') IS NOT NULL DROP TABLE #ssd_permanence;

-- Create structure
CREATE TABLE #ssd_ssd_permanence (
...
-- Insert data
INSERT INTO #ssd_ssd_permanence (
    perm_table_id,
    perm_person_id,
    perm_cla_id,
...
BEGIN
    -- -- Add constraint(s)
    -- ALTER TABLE #ssd_ssd_permanence ADD CONSTRAINT FK_perm_person_id
    -- FOREIGN KEY (perm_person_id) REFERENCES #ssd_ssd_cla_episodes(clae_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_perm_person_id            ON #ssd_ssd_permanence(perm_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_perm_adm_decision_date    ON #ssd_ssd_permanence(perm_adm_decision_date);
    CREATE NONCLUSTERED INDEX idx_ssd_perm_order_date           ON #ssd_ssd_permanence(perm_permanence_order_date);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_professionals', 'U') IS NOT NULL DROP TABLE #ssd_ssd_professionals;
...
-- Create structure
CREATE TABLE #ssd_ssd_professionals (
...
-- Insert data
INSERT INTO #ssd_ssd_professionals (
...
BEGIN
    ...
    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_prof_staff_id                 ON #ssd_ssd_professionals (prof_staff_id);
    CREATE NONCLUSTERED INDEX idx_ssd_prof_social_worker_reg_no ON #ssd_ssd_professionals(prof_social_worker_registration_no);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_department', 'U') IS NOT NULL DROP TABLE #ssd_ssd_department;
IF OBJECT_ID('tempdb..#ssd_department', 'U') IS NOT NULL DROP TABLE #ssd_department;

-- Create structure
CREATE TABLE #ssd_ssd_department (
...
-- Insert data
INSERT INTO #ssd_ssd_department (
    dept_team_id,
    dept_team_name,
    dept_team_parent_id,
...
BEGIN
    -- -- Add constraint(s)
    -- ALTER TABLE #ssd_ssd_department ADD CONSTRAINT FK_dept_team_parent_id 
    -- FOREIGN KEY (dept_team_parent_id) REFERENCES #ssd_ssd_department(dept_team_id);

    -- Create index(es)
    CREATE INDEX idx_dept_team_id ON #ssd_ssd_department (dept_team_id);
END
...
-- Check if exists & drop
IF OBJECT_ID('ssd_development.ssd_involvements', 'U') IS NOT NULL DROP TABLE #ssd_ssd_involvements;
IF OBJECT_ID('tempdb..#ssd_involvements', 'U') IS NOT NULL DROP TABLE #ssd_involvements;
 
-- Create structure
CREATE TABLE #ssd_ssd_involvements (
...
-- Insert data
INSERT INTO #ssd_ssd_involvements (
...
BEGIN
    ...
    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_invo_person_id            ON #ssd_ssd_involvements (invo_person_id);
    CREATE NONCLUSTERED INDEX idx_invo_professional_role_id ON #ssd_ssd_involvements (invo_professional_role_id);
    CREATE NONCLUSTERED INDEX idx_ssd_invo_start_date       ON #ssd_ssd_involvements(invo_involvement_start_date);
    CREATE NONCLUSTERED INDEX idx_ssd_invo_end_date         ON #ssd_ssd_involvements(invo_involvement_end_date);
    CREATE NONCLUSTERED INDEX idx_ssd_invo_referral_id      ON #ssd_ssd_involvements(invo_referral_id);
END
...
-- Check if exists, & drop 
IF OBJECT_ID('ssd_development.linked_identifiers', 'U') IS NOT NULL DROP TABLE #ssd_ssd_linked_identifiers;
IF OBJECT_ID('tempdb..#ssd_linked_identifiers', 'U') IS NOT NULL DROP TABLE #ssd_linked_identifiers;

-- Create structure
CREATE TABLE #ssd_ssd_linked_identifiers (
...
INSERT INTO #ssd_ssd_linked_identifiers (
    link_person_id, 
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_linked_identifiers ADD CONSTRAINT FK_link_to_person 
    FOREIGN KEY (link_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_link_person_id        ON #ssd_ssd_linked_identifiers(link_person_id);
    CREATE NONCLUSTERED INDEX idx_ssd_link_valid_from_date  ON #ssd_ssd_linked_identifiers(link_valid_from_date);
    CREATE NONCLUSTERED INDEX idx_ssd_link_valid_to_date    ON #ssd_ssd_linked_identifiers(link_valid_to_date);
END
...
-- Check if exists, & drop 
IF OBJECT_ID('ssd_development.s251_finance', 'U') IS NOT NULL DROP TABLE #ssd_ssd_s251_finance;
IF OBJECT_ID('tempdb..#ssd_s251_finance', 'U') IS NOT NULL DROP TABLE #ssd_s251_finance;

-- Create structure
CREATE TABLE #ssd_ssd_s251_finance (
...
-- INSERT INTO #ssd_ssd_s251_finance (
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_s251_finance ADD CONSTRAINT FK_s251_to_cla_placement 
    FOREIGN KEY (s251_cla_placement_id) REFERENCES #ssd_ssd_cla_placement(clap_cla_placement_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_s251_cla_placement_id ON #ssd_ssd_s251_finance(s251_cla_placement_id);
END
...
-- Check if exists, & drop 
IF OBJECT_ID('ssd_development.voice_of_child', 'U') IS NOT NULL DROP TABLE #ssd_ssd_voice_of_child;
IF OBJECT_ID('tempdb..#ssd_voice_of_child', 'U') IS NOT NULL DROP TABLE #ssd_voice_of_child;

-- Create structure
CREATE TABLE #ssd_ssd_voice_of_child (
...
-- -- Insert placeholder data [TESTING]
-- INSERT INTO #ssd_ssd_voice_of_child (
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_voice_of_child ADD CONSTRAINT FK_voch_to_person 
    FOREIGN KEY (voch_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_ssd_voice_of_child_voch_person_id ON #ssd_ssd_voice_of_child(voch_person_id);
END
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.pre_proceedings', 'U') IS NOT NULL DROP TABLE #ssd_ssd_pre_proceedings;
IF OBJECT_ID('tempdb..#ssd_pre_proceedings', 'U') IS NOT NULL DROP TABLE #ssd_pre_proceedings;

-- Create structure
CREATE TABLE #ssd_ssd_pre_proceedings (
...
-- INSERT INTO #ssd_ssd_pre_proceedings (
...
BEGIN
    -- Add constraint(s) #DtoI-1769
    ALTER TABLE #ssd_ssd_pre_proceedings ADD CONSTRAINT FK_prep_to_person 
    FOREIGN KEY (prep_person_id) REFERENCES #ssd_ssd_person(pers_person_id);

    -- Create index(es)
    CREATE NONCLUSTERED INDEX idx_prep_person_id                ON #ssd_ssd_pre_proceedings (prep_person_id);
    CREATE NONCLUSTERED INDEX idx_prep_pre_pro_decision_date    ON #ssd_ssd_pre_proceedings (prep_pre_pro_decision_date);
    CREATE NONCLUSTERED INDEX idx_prep_legal_gateway_outcome    ON #ssd_ssd_pre_proceedings (prep_legal_gateway_outcome);
END
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.ssd_send') IS NOT NULL DROP TABLE #ssd_ssd_send;
IF OBJECT_ID('tempdb..#ssd_send') IS NOT NULL DROP TABLE #ssd_send;

-- Create structure 
CREATE TABLE #ssd_ssd_send (
...
BEGIN
    -- Add constraint(s) -- #DtoI-1769
    ALTER TABLE #ssd_ssd_send ADD CONSTRAINT FK_send_to_person 
    FOREIGN KEY (send_person_id) REFERENCES #ssd_ssd_person(pers_person_id);
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.ssd_sen_need', 'U') IS NOT NULL DROP TABLE #ssd_ssd_sen_need  ;
IF OBJECT_ID('tempdb..#ssd_sen_need', 'U') IS NOT NULL DROP TABLE #ssd_sen_need  ;
 
 
-- Create structure
CREATE TABLE #ssd_ssd_sen_need (
...
-- -- Insert placeholder data
-- INSERT INTO #ssd_ssd_sen_need (senn_table_id, senn_active_ehcp_id, senn_active_ehcp_need_type, senn_active_ehcp_need_rank)
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.ssd_ehcp_requests', 'U') IS NOT NULL DROP TABLE #ssd_ssd_ehcp_requests ;
IF OBJECT_ID('tempdb..#ssd_ehcp_requests', 'U') IS NOT NULL DROP TABLE #ssd_ehcp_requests ;


-- Create structure
CREATE TABLE #ssd_ssd_ehcp_requests (
...
-- -- Insert placeholder data
-- INSERT INTO #ssd_ssd_ehcp_requests (ehcr_ehcp_request_id, ehcr_send_table_id, ehcr_ehcp_req_date, ehcr_ehcp_req_outcome_date, ehcr_ehcp_req_outcome)
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.ssd_ehcp_assessment', 'U') IS NOT NULL DROP TABLE #ssd_ssd_ehcp_assessment ;
IF OBJECT_ID('tempdb..#ssd_ehcp_assessment', 'U') IS NOT NULL DROP TABLE #ssd_ehcp_assessment ;


-- Create ssd_ehcp_assessment table
CREATE TABLE #ssd_ssd_ehcp_assessment (
...
-- -- Insert placeholder data
-- INSERT INTO #ssd_ssd_ehcp_assessment (ehca_ehcp_assessment_id, ehca_ehcp_request_id, ehca_ehcp_assessment_outcome_date, ehca_ehcp_assessment_outcome, ehca_ehcp_assessment_exceptions)
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.ssd_ehcp_named_plan', 'U') IS NOT NULL DROP TABLE #ssd_ssd_ehcp_named_plan;
IF OBJECT_ID('tempdb..#ssd_ehcp_named_plan', 'U') IS NOT NULL DROP TABLE #ssd_ehcp_named_plan;

-- Create structure
CREATE TABLE #ssd_ssd_ehcp_named_plan (
...
-- -- Insert placeholder data
-- INSERT INTO #ssd_ssd_ehcp_named_plan (ehcn_named_plan_id, ehcn_ehcp_asmt_id, ehcn_named_plan_start_date, ehcn_named_plan_ceased_date, ehcn_named_plan_ceased_reason)
-- VALUES ('SSD_PH', 'SSD_PH', '1900/01/01', '1900/01/01', 'SSD_PH');
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.ssd_ehcp_active_plans', 'U') IS NOT NULL DROP TABLE #ssd_ssd_ehcp_active_plans  ;
IF OBJECT_ID('tempdb..#ssd_ehcp_active_plans', 'U') IS NOT NULL DROP TABLE #ssd_ehcp_active_plans  ;

-- Create structure
CREATE TABLE #ssd_ssd_ehcp_active_plans (
...
-- -- Insert placeholder data
-- INSERT INTO #ssd_ssd_ehcp_active_plans (ehcp_active_ehcp_id, ehcp_ehcp_request_id, ehcp_active_ehcp_last_review_date)
...
SELECT * FROM #ssd_ssd_version_log WHERE is_current = 1;
...
BEGIN
    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_sen_need ADD CONSTRAINT FK_send_to_ehcp_active_plans
    FOREIGN KEY (senn_active_ehcp_id) REFERENCES #ssd_ssd_ehcp_active_plans(ehcp_active_ehcp_id);

    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_ehcp_active_plans ADD CONSTRAINT FK_ehcp_active_plans_requests
    FOREIGN KEY (ehcp_ehcp_request_id) REFERENCES #ssd_ssd_ehcp_requests(ehcr_ehcp_request_id);

    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_ehcp_named_plan ADD CONSTRAINT FK_ehcp_named_plan_assessment
    FOREIGN KEY (ehcn_ehcp_asmt_id) REFERENCES #ssd_ssd_ehcp_assessment(ehca_ehcp_assessment_id);

    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_ehcp_assessment ADD CONSTRAINT FK_ehcp_assessment_requests
    FOREIGN KEY (ehca_ehcp_request_id) REFERENCES #ssd_ssd_ehcp_requests(ehcr_ehcp_request_id);

    -- Add constraint(s)
    ALTER TABLE #ssd_ssd_ehcp_requests ADD CONSTRAINT FK_ehcp_requests_send
    FOREIGN KEY (ehcr_send_table_id) REFERENCES #ssd_ssd_send(send_table_id);
END
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.ssd_extract_log', 'U') IS NOT NULL DROP TABLE #ssd_ssd_extract_log;
IF OBJECT_ID('tempdb..#ssd_extract_log', 'U') IS NOT NULL DROP TABLE #ssd_extract_log;

-- Create logging structure
CREATE TABLE #ssd_ssd_extract_log (
...
BEGIN
    BEGIN TRY
        ...
        -- Insert log entry 
        INSERT INTO #ssd_ssd_extract_log (
            table_name, 
            schema_name, 
            status, 
            rows_inserted, 
...
SELECT * FROM #ssd_ssd_extract_log ORDER BY rows_inserted DESC;
...
--         SELECT 1 FROM #ssd_ssd_person p
--          WHERE CAST(p.pers_person_id AS INT) = fi.DIM_PERSON_ID -- #DtoI-1799
