...
WHERE EXISTS 
    ...
    WHERE p.pers_person_id = fc.DIM_PERSON_ID -- #DtoI-1799
    );
...
WHERE EXISTS 
    ...
    SELECT 1 
    FROM ssd_development.ssd_person p
    WHERE p.pers_person_id = pa.DIM_PERSON_ID -- #DtoI-1799
    );
...
WHERE EXISTS 
    ...
    SELECT 1 
    FROM ssd_development.ssd_person p
    WHERE p.pers_person_id = fd.DIM_PERSON_ID -- #DtoI-1799
    );
...
WHERE
    ...
    ( -- only ssd relevant records
        SELECT 1
        FROM ssd_development.ssd_person p
        WHERE p.pers_person_id = ims.DIM_PERSON_ID -- #DtoI-1799
    );
...
AND EXISTS
    ...
    SELECT 1
    FROM ssd_development.ssd_person p
    WHERE p.pers_person_id = fr.DIM_PERSON_ID
    );
...
-- drop/wipe persistent tmp/pre-processing table ready for further use in update section
...
-- moved to initial creation of ssd_person
...
WHERE 
    p.DIM_PERSON_ID IS NOT NULL
    AND p.DIM_PERSON_ID <> -1
    AND p.DIM_PERSON_ID NOT IN (SELECT CAST(pers_person_id AS INT) FROM ssd_development.ssd_person);
...
-- this tmp/pre-processing table already exists as defined in core script section
-- table is removed/clean-up after this update
INSERT INTO ssd_development.ssd_pre_cla_care_plan (
    FACT_FORM_ID,
    DIM_PERSON_ID,
    ANSWER_NO,
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.send') IS NOT NULL DROP TABLE ssd_development.ssd_send;
...
BEGIN
    ...
    PRINT 'Updating additional constraints';
...
GO -- Prev ALTER statements can otherwise hang
...
PRINT 'Starting extract logging process';
...
CREATE TABLE ssd_development.ssd_extract_log (
    ...
    has_fks              BIT,
    index_count          INT,
    null_count           FLOAT,          -- New: count of null values for each table
