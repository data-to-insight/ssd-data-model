...
-- store relevant static/threshold dates that counts can be calculated from/to
DECLARE @caseload_cutoff_date DATE; -- CASELOAD currently most recent previous Sept30th
-- DECLARE @xxxx_cutoff_date DATE;
...
-- Determine/Define date on which CASELOAD count required (Currently: most recent previous September 30th)
SET @caseload_cutoff_date = CASE 
...
DECLARE @TimeframeStartDate DATE = DATEADD(YEAR, -@ssd_timeframe_years, @caseload_cutoff_date);
...
LEFT JOIN (
    ...
    FROM 
        Child_Social.FACT_REFERRALS
    WHERE 
        REFRL_START_DTTM <= @caseload_cutoff_date AND 
        (REFRL_END_DTTM IS NULL OR REFRL_END_DTTM >= @caseload_cutoff_date) AND
...
-- Check if exists, & drop 
IF OBJECT_ID('ssd_development.linked_identifiers', 'U') IS NOT NULL DROP TABLE #ssd_linked_identifiers;
...
-- Check if exists, & drop 
IF OBJECT_ID('ssd_development.s251_finance', 'U') IS NOT NULL DROP TABLE #ssd_s251_finance;
...
-- Check if exists, & drop 
IF OBJECT_ID('ssd_development.voice_of_child', 'U') IS NOT NULL DROP TABLE #ssd_voice_of_child;
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.pre_proceedings', 'U') IS NOT NULL DROP TABLE #ssd_pre_proceedings;
...
ALTER TABLE #ssd_person ADD ssd_flag INT; -- [1== is filtered core record, 0==non-core record brought through as part of update]
Go
...
UPDATE #ssd_person SET ssd_flag = 1;
...
WHERE 
    p.DIM_PERSON_ID IS NOT NULL
    AND p.DIM_PERSON_ID <> -1
    AND p.DIM_PERSON_ID NOT IN (SELECT pers_person_id FROM #ssd_person);
...
DROP TABLE #ssd_pre_cla_care_plan;
...
-- Check if exists, & drop
IF OBJECT_ID('ssd_development.send') IS NOT NULL DROP TABLE #ssd_send;
...
CREATE TABLE #ssd_table_creation_log (
...
BEGIN
    BEGIN TRY
        ...
        -- Insert log entry 
        INSERT INTO #ssd_table_creation_log (
            table_name, 
            schema_name, 
            status, 
            rows_inserted, 
    ...
    BEGIN CATCH
        ...
        INSERT INTO #ssd_table_creation_log (table_name, schema_name, status, rows_inserted, table_size_kb, has_pk, has_fks, index_count, additional_detail, error_message)
