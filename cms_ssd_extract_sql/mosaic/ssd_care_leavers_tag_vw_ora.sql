define p_snapshot_date = to_date('13/02/2024', 'dd/mm/yyyy');
--
with pathway_plan_step_types as (
    select
        typ.workflow_step_type_id,
        typ.description
    from
        dm_workflow_step_types typ
    where
        typ.workflow_step_type_id in (
            498,     --'Pathway Plan'),
            500     --'Pathway Plan Review')       
        )
),
personal_advisor_rel_type_codes as (
    select
         prel.prof_rel_type_code
    from
        dm_prof_rel_types prel
    where
        prel.prof_rel_type_code in (
            'REL.PERSONALADVISOR'
        )
),
days_looked_after as ( 
	select
		cla.person_id,
		dbo.days_diff(
			case
				when coalesce(cla.end_date,dbo.future(null)) > &p_snapshot_date then
					cast(&p_snapshot_date as date)
				else
					cla.end_date
				end,
				case
	...
	where
		...
		cla.start_date <= &p_snapshot_date
		and
...
select 
	...
	case
		when (
			...
			case
				when dbo.f_person_age(p.DATE_OF_BIRTH,&p_snapshot_date) between 16 and 17 then
					case
						when exists (
								select
									1
								...
								where
									...
									and
									&p_snapshot_date between poc.START_DATE and dbo.future(poc.END_DATE)
							) then 
							'd) Eligible'
						else 
							'a) Relevant child'
					end
				when dbo.f_person_age(p.DATE_OF_BIRTH,&p_snapshot_date) between 18 and 21 then
					'b) Former relevant child'
				when dbo.f_person_age(p.DATE_OF_BIRTH,&p_snapshot_date) between 22 and 25 then
					case
						when exists (
								select
									1
								...
								where
									...
									and
									&p_snapshot_date between awkr.START_DATE and dbo.future(awkr.END_DATE)
								union
								select
									1
								from
								...
								where
									...
									and
									&p_snapshot_date between pr.START_DATE and dbo.future(pr.END_DATE)
							) then
							'b) Former relevant child'
					end
			end
			...
			case
				when not exists (
					...
					case
						when dbo.f_person_age(p.DATE_OF_BIRTH,&p_snapshot_date) between 16 and 21 then
							'c) Qualifying care leaver'
						when dbo.f_person_age(p.DATE_OF_BIRTH,&p_snapshot_date) between 21 and 25 then
							case
								when exists (
									select
										1
...
where
	...
	exists (
		...
		where
			...
			and
			( 
				poc.START_DATE <= &p_snapshot_date
				and
...
select
		...
		where
			...
			info.weighted_start = (
				...
				where
					...
					and
					info1.DATE_OF_SITUATION <= &p_snapshot_date
			)
	...
	(
		...
		where
			...
			and
			pwp.completed_on <= &p_snapshot_date
	...
	(
		...
		and
		&p_snapshot_date between wro.START_DATE and dbo.future(wro.END_DATE)
		...
		where
			...
			dbo.to_weighted_start(awkr.START_DATE,awkr.ALLOCATION_ID) = (
				...
				where
					...
					and
					&p_snapshot_date between awkr1.START_DATE and dbo.future(awkr1.END_DATE)
			)
			and
			dbo.to_weighted_start(wro.START_DATE,wro.WORKER_ROLE_ID) = (
				...
				where
					...
					and
					&p_snapshot_date between wro1.START_DATE and dbo.future(wro1.END_DATE)
			)
