...
from Mosaic.M.PERSON_NON_LA_LEGAL_STATUSES a
...
(
	Select
	t.prep_person_id,
	g.GroupID prep_plo_family_id,
	DENSE_RANK() OVER(PARTITION BY t.prep_person_id ORDER BY (CASE when g.SubGroupSoleSubject is null then 1 else 2 END), g.GroupID DESC, g.SubGroupID DESC) Rnk

	from #t t
	inner join Mosaic.D.PersonData p on t.prep_person_id = p.PersonID
	inner join Mosaic.D.Groups g on t.prep_person_id = g.SubjectID
			and COALESCE(g.GroupName,'zzz') not like '%Duplicate%'
...
from #t t
inner join Mosaic.M.PERSON_LEGAL_STATUSES a on t.prep_person_id = a.PERSON_ID
...
/*Number of CP Plans during referral period*/
UPDATE #t
...
(
	...
	t.prep_table_id,
	t.prep_person_id,
	COUNT(a.CPRegID) NoOfCPPlans

	from #t t
	inner join ChildrensReports.ICS.CPRegistrations a on t.prep_person_id = a.PersonID
		and a.RegType = 'P'
		and a.RegStart between t.CINPeriodStart and t.CINPeriodEnd
...
(
	...
	from #t t
	inner join Mosaic.M.PERSON_NON_LA_LEGAL_STATUSES a on t.prep_person_id = a.PERSON_ID
		and a.LEGAL_STATUS = 'PLOPRE'
